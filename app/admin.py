...
class LinkAdmin(ModelAdmin):
    ...
    def linked_title(self, link: Link):
        text = (
            "[%s] %s" % (link.metadata["subreddit"], link.title)
            if link.reddit_id
            else link.title
        )
...
def embed_reddit_media_embed(md: Mapping) -> Optional[str]:
    for embed in filter(
        None, (md.get(e) for e in ("media_embed", "secure_media_embed"))
    ):
        if not (content := embed.get("content")):
            continue
        return format_html(
            '<div style="position: relative; overflow: hidden; padding-top: {}%">{}</div>',
            get_iframe_padding(embed),
            mark_safe(content),
        )
...
def get_reddit_video(md: Mapping) -> Optional[Mapping]:
    return first_not_none(
        ((md.get(k) or {}).get("reddit_video") for k in ("media", "secure_media"))
    )
